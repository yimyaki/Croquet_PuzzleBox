...
class PuzzleBoxModel extends Croquet.Model {
    init(options) {
        ...
        this.initialize([this.pic_width,this.pic_height]);
    }
    ...
    initialize(metadata){
      ...
      for (let i = 0; i < Math.floor(this.pic_width/PieceSize); i++){
        ...
        for (const id of this.pieces.keys()){
          this.removePiece({viewId: this.id, id: id})
        }
        ...
        this.children.forEach(obj => {
            this.pieces.set(this.currentId++, {
                x: obj.pos[0],// * (this.width - BallDiameter * 2),
                y: obj.pos[1],// - (ftop(stof(obj.n)) * (this.height - BallDiameter * 2)) - BallDiameter * 2,
                grabbed: null,
                goal: obj.goal,
                backPos: obj.backPos,
                initPos:obj.pos,
...
class PuzzleBoxView extends Croquet.View {
    constructor(model) {
        ...
        this.pic_url = "url(https://s3-us-west-2.amazonaws.com/s.cdpn.io/29841/dog.jpg)";
    ...
    reset(){
      ...
      this.publish(this.id, 'reload', [this.pic_width,this.pic_height]);
    }
    ...
    async showImage(asset) {
        ...
        this.getMeta(new_url);
        console.log('ok');
        this.publish(this.viewId, "reload", [this.pic_width,this.pic_height]);
        console.log('maybe?')
        this.initializePieces();
    }
...
Croquet.Session.join({
  ...
  /* (more join options)... */
  name: "unnamed8",//Croquet.App.autoSession(),
  password: "secret8",//Croquet.App.autoPassword(),
